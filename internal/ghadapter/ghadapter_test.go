...
func TestStatusForFailsClosedOnMissingActive(t *testing.T) {
	...
	writeExe(t, filepath.Join(bin, "gh"), `#!/bin/sh
printf '{"hosts":{"github.com":[{"state":"success","active":false,"host":"github.com","login":"user"}]}}\n'
`)
	t.Setenv("PATH", bin+string(os.PathListSeparator)+os.Getenv("PATH"))
...
func TestEnsureAccountRequiresSuccessfulState(t *testing.T) {
	st := Status{Active: "ok", Accounts: []Account{{Login: "bad", State: "failure"}, {Login: "ok", State: "success"}}}
